...
public class ProgramVariantFactory {
    ...
    private ProgramVariant createProgramInstance(List<SuspiciousCode> suspiciousList, int idProgramInstance) {
        ...
        if (!suspiciousList.isEmpty()) {
            ...
            String buggyMethodPath = this.projectFacade.getProperties().getBuggyMethods();
            Collection<String> buggyMethods = new LinkedList();

            if (buggyMethodPath != null) {
                File f = new File(buggyMethodPath);

                try {
                    for (String s : Files.readAllLines(f.toPath())) {
                        String buggyMethod = s.replaceAll(Pattern.quote("^"), "").trim();
                        buggyMethods.add(buggyMethod);

                    }
                } catch (Exception e) {
                    System.out.println("Could not properly load/set buggy methods!");
                    System.out.println(e.getMessage());
                }
            }
            for (SuspiciousCode suspiciousCode : suspiciousList) {
                boolean keepGoing = false;
                String c = suspiciousCode.getClassName();
                String m = suspiciousCode.getMethodName();

                if (buggyMethodPath != null) {
                    for (String s : buggyMethods) {
                        if (s.contains(String.format("%s:%s", c, m))) {
                            System.out.println(String.format("Buggy method found - %s:%s", c, m));
                            keepGoing = true;
                        }
                    }
                } else {
                    System.out.println("No buggy methods detected");
                }
