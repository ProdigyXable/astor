 ...
 *
 */
public class MutationalExhaustiveRepair_Profl extends MutationalExhaustiveRepair {
    ...
    public MutationalExhaustiveRepair_Profl(MutationSupporter mutatorExecutor, ProjectRepairFacade projFacade) throws JSAPException, Exception {
        ...
        log.info(String.format("Creating Profl-based jMutRepair %s, %s, %s", proflMethod, proflTest, proflFailing));
    }
    ...
    public VariantValidationResult validateInstance(ProgramVariant variant) {
        ...
        if (ConfigurationProperties.getPropertyBool("testbystep") && validationResult instanceof TestCasesProgramValidationResult) {
            ...
            for (OperatorInstance lastOperator : variant.getAllOperations()) {
                ...
                Double susValue = profl.getGeneralMethodSusValues().getOrDefault(buggyMethod, new Double(0));
                ...
                if (buggyMethod != null && results.individualTestsProcessed == true) {
                    ...
                    JUnitProcessValidator.saveTestInformation();
